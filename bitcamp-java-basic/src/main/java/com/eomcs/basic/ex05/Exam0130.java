...
public class Exam0130 {
  public static void main(String[] args) {
    ...
    //z = (x + y); // 컴파일 오류!컴파일러는 현재문장을 기계어로 바꿀수 있냐
    //없냐를 보고 컴파일하기때문에 x,y를 컴파일 할때 값은 중요하지않다
    //변수가있으면 무조건 4바이트기때문에 안됨
    //자바는 기본연산을 4바이트로 계산
    //리터럴은 변수가아니기때믄에 가능
    ...
    s3 = s2; // OK!
    //s3 = s1 + s2; // 컴파일 오류!
    //자바의 기본연산단위는 4바이트이기때문에 직접연산할 수 없고
    //임시로 4바이트 메모리를 만들어 그메모리에 값을 저장 후 연산을수행한다.
    //따라서 4바이트 메모리를 2바이트에 넣을 수 없기 때문에 컴파일 오류가
    //생긴다.
    int s4 = s1 + s2;
